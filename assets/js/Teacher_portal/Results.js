...
function displayalert(type, message) {
    ...
    alertdiv.innerHTML = `
                        <i class="fas fa-circle-check h6 me-2"></i>
                        <div>
                           ${message}
                        </div>
                        `
...
async function readJsonFromFile() {
    try {
        ...
        const existingData = localStorage.getItem('studentscalculatedData');
        if (existingData) {
            const parsedData = JSON.parse(existingData);
            parsedData.studentsWithCalculatedFields = studentsWithCalculatedFields;

            localStorage.setItem('studentscalculatedData', JSON.stringify(parsedData));
        } else {
            const newData = {
                studentsWithCalculatedFields: studentsWithCalculatedFields,
            };
            localStorage.setItem('studentscalculatedData', JSON.stringify(newData));
        }
...
function submitallstudentresult(data, classdata) {
    ...
    fetch(`/TMS/submitallstudentresult/`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRFToken': csrftoken
...
class DataTable {
    ...
    renderRows(rows) {
        ...
        // check the rows that are hidden and make sure they remain hidden after populating the rows
        ...
        //         this.handleCheckboxChange({ target: checkbox }); // Trigger the change event
        //     }
        // });
